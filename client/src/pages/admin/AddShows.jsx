...
const AddShows = () => {
  ...
  const fetchtNowPlayingMovies = async ()=> {
  ...
  useEffect(()=>{
    fetchtNowPlayingMovies();
  },[])

  return nowPlayingMovies.length > 0 ? (
    <>
      <Title text1="Add" text2="Shows"/>
      <p className='mt-10 text-lg fotn-medium'>Now Playing Movies</p>
          ...
          {nowPlayingMovies.map((movie) => (
            <div onClick={() => setSelectedMovie(movie.id)} key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`}>
                <div className='relative rounded-lg overflow-hidden'>
                  <img className='w-full object-cover brightness-90' src={movie.poster_path} alt="" />
      ...
      <div className='mt-6'>
        <label className='block tet-sm font-medium mb-2'>Select Date and Time</label>
